...
uniform float _randomRange; // 追加: ランダム回転の制御用
...
// const float SEPIA_INTENSITY = 0.11;
...
vec3 applyFilterA(vec3 color, vec2 uv) {
    ...
    color = mix(color, SEPIA_COLOR * lum, _sepia);
...
void main() {
    ...
    for (int j = -4; j <= 0; ++j) {
        for (int i = -4; i <= 0; ++i) {
            float rand_angle = rand(vec2(float(i), float(j)) + uv + _time) * _randomRange;
            vec2 offset = vec2(float(i), float(j)) * src_size;
            offset = rotate2d(rand_angle) * offset;
            c = texture(uTexture, uv + offset).rgb;
            m0 += c;
            s0 += c * c;
        }
    }
